...
export default function InfraVisual({ className = "" }: { className?: string }) {
  ...
  return (
    <div className={`relative w-full h-full ${className}`}>
      {/* window chrome */}
      <div className="glass-strong rounded-t-2xl px-4 py-2.5 flex items-center gap-1.5 text-[11px] font-mono">
        <span className="h-2.5 w-2.5 rounded-full bg-red-400/80" />
        <span className="h-2.5 w-2.5 rounded-full bg-yellow-400/80" />
        <span className="h-2.5 w-2.5 rounded-full bg-emerald-400/80" />
        <span className="ml-2 text-ink/45">~/devopsdays-pk · control-center</span>
        <span className="ml-auto inline-flex items-center gap-1.5 text-[10px] uppercase tracking-[0.22em] text-neon-glow">
      ...
      {/* body */}
      <div className="glass-strong border-t-0 rounded-b-2xl p-4 sm:p-5 space-y-5 overflow-hidden">
        {/* KPI tiles */}
        ...
        <div className="relative">
          ...
          <div className="relative">
            {/* rail */}
            <div className="absolute inset-x-3 top-1/2 -translate-y-1/2 h-px bg-neon/25" />
            {/* moving glow */}
            {!reduce && (
              <motion.div
                className="absolute top-1/2 -translate-y-1/2 h-1 w-12 rounded-full bg-gradient-to-r from-transparent via-neon-glow to-transparent"
                animate={{ left: ["-10%", "100%"] }}
                transition={{
                  duration: 3,
                  repeat: Infinity,
                ...
                return (
                  <div key={s.label} className="flex flex-col items-center gap-2 z-10">
                    <motion.div
                      animate={{
                        boxShadow: isActive
                          ? "0 0 20px rgba(192,132,252,0.85), 0 0 40px rgba(168,85,247,0.45)"
                          : "0 0 0 rgba(0,0,0,0)",
                      }}
        ...
        <div className="flex flex-wrap gap-1.5">
          {STACK.map((s) => (
            <span
              key={s.label}
              className="inline-flex items-center gap-1.5 rounded-full border border-neon/25 bg-neon/[0.06] px-2.5 py-1 text-[10px] font-mono uppercase tracking-[0.2em] text-ink/70 hover:bg-neon/15 hover:text-ink hover:border-neon/45 transition-colors"
            >
        ...
        {/* log feed */}
        <div className="rounded-lg bg-black/40 border border-neon/15 p-3 font-mono text-[10.5px] leading-relaxed h-[110px] overflow-hidden relative">
...
function Kpi({
  ...
  return (
    <div className="relative rounded-lg border border-neon/20 bg-neon/[0.04] px-2.5 py-2 overflow-hidden">
      ...
      <div className="text-[9px] font-mono uppercase tracking-[0.2em] text-ink/45">
        {label}
      </div>
      <motion.div
        key={value}
        ...
        className={`mt-0.5 font-display font-bold text-lg sm:text-xl tabular-nums ${accent}`}
      >
        {value}
      </motion.div>
    </div>
